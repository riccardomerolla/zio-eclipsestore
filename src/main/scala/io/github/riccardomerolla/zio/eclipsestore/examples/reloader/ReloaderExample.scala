...
object ReloaderExample:
  private val rootDescriptor = io.github.riccardomerolla.zio.eclipsestore.domain.RootDescriptor[DataRoot](
    id = "reloader-root",
  ...
  private def program: ZIO[EclipseStoreService, EclipseStoreError, List[String]] =
    for
      ...
      _ <- EclipseStoreService.execute(
             io.github.riccardomerolla.zio.eclipsestore.domain.Query.Custom[Unit](
               "reload-flat",
               ctx =>
                 ctx.storageManager.foreach { sm =>
                   val pm       = sm.persistenceManager().asInstanceOf[org.eclipse.serializer.persistence.types.PersistenceManager[Binary]]
